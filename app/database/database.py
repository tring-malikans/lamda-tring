...
from database.database_helper import student_helper, admin_helper ,order_helper, coin_helper ,user_helper ,indi_helper
import pymongo
...
indi_collection = database.get_collection('indicators')
taapi_collection = database.get_collection('taapidata')
...
async def retrieve_user(id: str) -> dict:
    ...
    if user:
        return user_helper(user)
# Get ONe balace
async def retrieve_user_account_balance(id: str) -> dict:
    user = user_collection.find_one({"_id": ObjectId(id)})
    binace_client = Client(user['api_key'], user['secret_key'])
    value=binace_client.get_asset_balance(asset="BUSD")
    print(value)
    if value:
        return value
# Get account select
async def retrieve_user_account(name: str) -> dict:
    users = []
    for user in user_collection.find():
        users.append(user_helper(user))
    newUsers=[]
    for user in users:
        if user['accountSelect']==name:
...
# Add indi
async def add_indi(indi_data: dict) -> dict:
    indi = indi_collection.insert_one(indi_data)
    new_indi =  indi_collection.find_one({"_id": indi.inserted_id})
    # print(indi.inserted_id)
    print(new_indi)
    new_indi['_id']=str(new_indi['_id'])
    return new_indi
# Get All
async def retrieve_indi():
    indis = []
    for indi in indi_collection.find():
        if indi['status']:
            indi['_id']=str(indi['_id'])
            indis.append(indi)
    return indis

async def retrieve_indi_tapi(id):
    indis = []
    for indi in taapi_collection.find():
        if indi['indi_id']==id:
            indi['_id']=str(indi['_id'])
            indis.append(indi)
    return indis
...
# Update
async def update_indi_data(id: str, data: dict):
    indi = indi_collection.find_one({"_id": ObjectId(id)})
    if indi:
        indi_collection.update_one({"_id": ObjectId(id)}, {"$set": data})
        return True
...
async def retrieve_coins():
    coins = []
    for coin in coins_collection.find():
